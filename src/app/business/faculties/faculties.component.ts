...
export class FacultiesComponent implements OnInit {
  ...
  loadFaculties() {
    this.http.get<Faculty[]>('http://192.168.1.109/facultades/').subscribe(
      data => {
        this.faculties = data;
      },
      error => {
  ...
  deleteFaculty() {
    if (this.facultyIdToDelete !== null) {
      this.http.delete(`http://192.168.1.109/facultades/${this.facultyIdToDelete}`).subscribe(
        () => {
  ...
  saveFaculty(faculty: Faculty) {
    if (this.isEditMode) {
      ...
      this.http.put<Faculty>(`http://192.168.1.109/facultades/${faculty.id}`, faculty).subscribe(
        response => {
      ...
      this.http.post<Faculty>('http://192.168.1.109/facultades/', faculty).subscribe(
        response => {
