...
export class MateriasComponent implements OnInit {
  ...
  loadMaterias() {
    this.http.get<Materia[]>('http://192.168.1.109/materias/').subscribe(
      data => {
        this.materias = data;
      },
      error => {
  ...
  deleteMateria() {
    if (this.materiaIdToDelete !== null) {
      this.http.delete(`http://192.168.1.109/materias/${this.materiaIdToDelete}`).subscribe(
        () => {
  ...
  saveMateria(materia: Materia) {
    if (this.isEditMode) {
      ...
      this.http.put<Materia>(`http://192.168.1.109/materias/${materia.id}`, materia).subscribe(
        response => {
      ...
      this.http.post<Materia>('http://192.168.1.109/materias/', materia).subscribe(
        response => {
