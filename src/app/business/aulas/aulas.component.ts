...
export class AulasComponent implements OnInit {
  ...
  loadAulas() {
    this.http.get<Aula[]>('http://192.168.1.109/aulas/').subscribe(
      data => {
        this.aulas = data;
      },
      error => {
  ...
  deleteAula() {
    if (this.aulaIdToDelete !== null) {
      this.http.delete(`http://192.168.1.109/aulas/${this.aulaIdToDelete}`).subscribe(
        () => {
  ...
  saveAula(aula: Aula) {
    if (this.isEditMode) {
      ...
      this.http.put<Aula>(`http://192.168.1.109/aulas/${aula.id}`, aula).subscribe(
        response => {
      ...
      this.http.post<Aula>('http://192.168.1.109/aulas/', aula).subscribe(
        response => {
