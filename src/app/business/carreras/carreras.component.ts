...
export class CarrerasComponent implements OnInit {
  ...
  loadCarreras() {
    this.http.get<Carrera[]>('http://192.168.1.109/carreras/').subscribe(
      data => {
        this.carreras = data;
      },
      error => {
  ...
  deleteCarrera() {
    if (this.carreraIdToDelete !== null) {
      this.http.delete(`http://192.168.1.109/carreras/${this.carreraIdToDelete}`).subscribe(
        () => {
  ...
  saveCarrera(carrera: Carrera) {
    if (this.isEditMode) {
      ...
      this.http.put<Carrera>(`http://192.168.1.109/carreras/${carrera.id}`, carrera).subscribe(
        response => {
      ...
      this.http.post<Carrera>('http://192.168.1.109/carreras/', carrera).subscribe(
        response => {
