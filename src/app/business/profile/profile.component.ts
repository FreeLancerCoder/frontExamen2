...
export class ProfileComponent implements OnInit {
  ...
  loadUsers() {
    this.http.get<User[]>('http://192.168.0.15/usuarios/').subscribe(
      data => {
        this.users = data;
      },
      error => {
  ...
  deleteUser() {
    if (this.userIdToDelete !== null) {
      this.http.delete(`http://192.168.0.15/usuarios/${this.userIdToDelete}`).subscribe(
        () => {
  ...
  saveUser(user: User) {
    ...
    if (this.isEditMode) {
        ...
        this.http.put<User>(`http://192.168.0.15/usuarios/${user.id}`, userPayload).subscribe(
            response => {
        ...
        this.http.post<User>('http://192.168.0.15/usuarios/', userPayload).subscribe(
            response => {
